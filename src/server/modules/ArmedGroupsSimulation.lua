...
local EmberMovementService = require(script.Parent.EmberMovementService)
...
function ArmedGroupsSimulation.start(player, locationName, difficulty, services, state)
	...
	local base = SimulationBase.new("ArmedGroupsSimulation")

	-- === Define simulation-specific logic ===
	local function onArmedGroupsSimulationStart(player, session, services)
		local spawnedNPCs = {}

		-- Override cleanup to include armed-groups-specific cleanup
		...
		-- === Armed-groups-specific dialog flow ===
		DialogService.send(
			player,
			"Warning",
	...
	base:activateSimulation(player, locationName, difficulty, services, state, onArmedGroupsSimulationStart)
end
