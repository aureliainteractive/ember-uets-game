...
local function stepPoints(time, maxTime)
	if time <= maxTime * 0.50 then return POINTS_PER_STEP.PERFECT
	elseif time <= maxTime * 0.70 then return POINTS_PER_STEP.EXCELLENT
	elseif time <= maxTime * 1.00 then return POINTS_PER_STEP.COMPLETED
	elseif time <= maxTime * 1.30 then return POINTS_PER_STEP.LATE
	else return POINTS_PER_STEP.VERY_LATE
	end
end
...
local function getRank(ratio)
	for _, entry in ipairs(RANKS) do
		if ratio >= entry.min then return entry.rank end
	end
	return "D"
end
...
local function getPrecision(waypointTimes, maxTimes)
	...
	if totalUsed <= 0 then return 100 end
	return math.clamp(
		math.floor((totalBudget / totalUsed) * 100),
		0, 100
	)
end
...
function ResultsSystem.compute(session, simType, locationName, difficulty)
	if not session
		or not session.waypointTimes
		or #session.waypointTimes == 0 then
	...
	for _, t in ipairs(times) do totalElapsed += t end
...
function ResultsSystem.show(player, session, simType,
                             locationName, difficulty, mainLobbySpawn)
	local payload = ResultsSystem.compute(
		session, simType, locationName, difficulty
	)

	if not payload then
		warn("[ResultsSystem] No se pudo calcular resultados para "
	...
	print(string.format(
		...
		player.Name, simType, payload.totalPoints, payload.rank
	))
end
...
returnToLobbyEvent.OnServerEvent:Connect(function(player)
	local mainLobbySpawn = workspace
		:WaitForChild("Spawnpoints")
		:WaitForChild("MainLobby")
	NavigationUtils.teleportPlayer(player, mainLobbySpawn)
