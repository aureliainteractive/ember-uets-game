...
local NexusVRCharacterModel =
	ReplicatedStorage:WaitForChild("NexusVRCharacterModel")

local VRInputService =
	require(NexusVRCharacterModel.State.VRInputService).GetInstance()
...
local function fireToggleFromHand(hand)
	...
	local handCF =
		hand == "Left"
		and inputs[Enum.UserCFrame.LeftHand]
		or inputs[Enum.UserCFrame.RightHand]

	local worldCF =
		cam:GetRenderCFrame()
		* inputs[Enum.UserCFrame.Head]:Inverse()
		* handCF

	local result = Workspace:Raycast(
		worldCF.Position,
		worldCF.LookVector * MAX_DISTANCE
	)
